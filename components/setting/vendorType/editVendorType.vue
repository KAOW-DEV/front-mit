    ...
    <form @submit.prevent="checkSave">
      <v-card>
        <v-card-title primary-title>
          แก้ไขข้อมูลประเภทผู้จำหน่าย
          <v-spacer></v-spacer>
          <v-btn icon @click="$emit('closeDialogEditVendorType')">
        ...
        <v-card-actions>
          ...
          <v-btn color="error" @click="$emit('closeDialogEditVendorType')"
            >ปิด</v-btn
          >
          <v-spacer></v-spacer>
        </v-card-actions>
    ...
    async checkSave() {
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.$emit("closeDialogEditVendorType");
          this.editVendorType();
        }
      });
    },

    async editVendorType() {
      await this.$axios
        .put("/vendor-types/" + this.itemVendorType.id, {
          vendor_type_name: this.itemVendorType.vendor_type_name,
        })
        .then((res) => {
          console.log("editVendorType", res.data);
