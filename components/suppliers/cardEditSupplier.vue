    ...
    <form @submit.prevent="checkSave">
      <v-card>
        <v-card-title primary-title>
          แก้ไขข้อมูลผู้ผลิต
          <v-spacer></v-spacer>
          <v-btn icon @click="$emit('closeDialogEditSupplier')">
        ...
        <v-card-actions>
          ...
          <v-btn
            class="mx-3"
            color="error"
            @click="$emit('closeDialogEditSupplier')"
            >ปิด</v-btn
          >
          <v-spacer></v-spacer>
          <v-btn x-large icon color="error" @click="checkDelete">
...
export default {
  ...
  methods: {
    async loadData() {
      ...
      await this.getItemsAmphures();
      await this.getItemsDistricts();
    ...
    async getDate() {
      this.recordDate = moment(this.itemSupplier.created_at)
        .add(543, "year")
        .format("DD/MM/YYYY HH:mm:ss");

      this.revisionDate = moment(this.itemSupplier.updated_at)
        .add(543, "year")
        .format("DD/MM/YYYY HH:mm:ss");
    },
    ...
    async checkSave() {
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          this.$emit("closeDialogEditSupplier");
          this.updateSupplier();
        }
      });
    },
    ...
    async updateSupplier() {
      await this.$axios
        .put("/suppliers/" + this.itemSupplier.id, {
